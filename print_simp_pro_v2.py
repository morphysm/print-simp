# PrintSimp Pro v2 - Windows Screenshot Tool
...
# Tray menu includes "Start with Windows" toggle (writes to HKCU registry, no admin needed)
...
def _set_startup(enable):
    with winreg.OpenKey(winreg.HKEY_CURRENT_USER, _STARTUP_KEY, 0, winreg.KEY_SET_VALUE) as key:
        if enable:
            winreg.SetValueEx(key, _APP_NAME, 0, winreg.REG_SZ, _get_startup_cmd())
        else:
            try:
...
def _start_tray(listener):
    ...
    def toggle_startup(icon, item):
        _set_startup(not _is_startup_registered())
    ...
    icon = pystray.Icon(
        ...
        menu=pystray.Menu(
            ...
            pystray.MenuItem("Start with Windows", toggle_startup, checked=lambda item: _is_startup_registered()),
